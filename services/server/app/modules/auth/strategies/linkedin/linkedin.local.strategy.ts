...
export class LinkedInLocalStrategy extends OAuth2Strategy {
  ...
  public userProfile(accessToken: string, done: (err?: Error | null, profile?: LinkedInOidcProfile) => void): void {
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    this._oauth2.get(this.profileUrl, accessToken, (err: unknown, body?: string | Buffer, _res?: unknown) => {
      if (err) {
        return done(new InternalOAuthError('Failed to fetch user profile', err));
      }

      if (!body) {
        return done(new InternalOAuthError('Failed to fetch user profile, empty response body', null));
      }
  ...
  public authorizationParams(options: { state?: string }): object {
